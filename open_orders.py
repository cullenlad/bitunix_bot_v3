...
def req(method,path,query=None,body=None):
    ...
    url=BASE+path
    r = requests.request(method,url,params=query,data=bd,headers=hd,timeout=15)
    t=r.text
    try: j=r.json()
...
def list_open(sym):
    try:
        j=req("POST","/api/v1/futures/trade/open_orders",{},{"symbol":sym})
        return j.get("data") or []
    except:
        j=req("GET","/api/v1/futures/trade/open_orders",{"symbol":sym},{})
        return j.get("data") or []
if __name__=="__main__":
    print(json.dumps(list_open(SYM),indent=2))
