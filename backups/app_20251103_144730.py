...
def dryrun():
    set_env('LIVE','0'); return run_cmd(['/opt/bitunix-bot/venv/bin/python3','gridbot_v2.py'])

@app.route('/golive',methods=['POST'])
def golive():
    set_env('LIVE','1'); return run_cmd(['/opt/bitunix-bot/venv/bin/python3','gridbot_v2.py'])
...
def trigger_live():
    ...
    with open(log_path, "a") as log:
        subprocess.Popen(
            ["/opt/bitunix-bot/venv/bin/python3", "/opt/bitunix-bot/gridbot_v2.py"],
            stdout=log,
            stderr=log,
            env=env,
        )
