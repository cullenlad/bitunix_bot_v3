...
def change_leverage(symbol, leverage):
    ...
    try:
        j = api_post("/api/v1/futures/account/change_leverage", body)
...
def place_order(symbol, side, price, size):
    ...
    try:
        j = api_post("/api/v1/futures/order/place_order", body)
...
def apiapi_post(endpoint, data):
    ...
    for nonce in nonces:
        for method in ("POST", "post"):
            clean = norm_path(endpoint)
            url = f"{base}/{clean}"
            for bj in (
                json.dumps(data, separators=(',', ':')),
                json.dumps(data, separators=(',', ':'), sort_keys=True),
            ):
                pre = f"{ts}{nonce}{method}{clean}{bj}"
                raw = hmac.new(API_SECRET.encode(), pre.encode(), hashlib.sha256).digest()
                for sig in (raw.hex(), base64.b64encode(raw).decode()):
                    headers = {
                        "api-key": API_KEY,
                        "sign": sig,
                        "signType": "HmacSHA256",
                    ...
                    try:
                        resp = requests.post(url, headers=headers, data=bj, timeout=12)
                        try:
                            j = resp.json()
                        except Exception:
                            j = {"status": resp.status_code, "text": resp.text}
                        logging.info(f"POST /{clean} => {j}")
                        if isinstance(j, dict) and j.get("code") in (0, 200):
                            return j
                        last = j
                    except Exception as e:
                        logging.error(f"api_post error: {e}")
    return last or {"code": -1, "msg": "no response"}
